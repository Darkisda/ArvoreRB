...
export class RbTree {
  ...
  preOrder() {
    let res = ''
    this.preOrderNode(this.root, res)
  }

  preOrderNode(node, res) {
    if (node !== null) {
      res += 'Valor do No: ' + node.key + ' Cor: ' + node.color + ' -> '
      console.log(res)
      this.preOrderNode(node.left, res)
      this.preOrderNode(node.right, res)
    }
  }
}
